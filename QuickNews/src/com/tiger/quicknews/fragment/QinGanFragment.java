...
public class QinGanFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    protected void initView() {
        ...
        loadData(getCommonUrl(index + "", Url.QingGanId));
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("QinGanFragment" + currentPagte);
    ...
    public void onRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                currentPagte = 1;
                isRefresh = true;
                loadData(getCommonUrl(0 + "", Url.QingGanId));
                url_maps.clear();
    ...
    public void getResult(String result) {
        getMyActivity().setCacheStr("QinGanFragment" + currentPagte, result);
        if (isRefresh) {
            isRefresh = false;
            newAdapter.clear();
            listsModles.clear();
