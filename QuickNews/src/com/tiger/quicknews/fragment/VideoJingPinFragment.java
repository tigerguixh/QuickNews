...
public class VideoJingPinFragment extends BaseFragment implements
    ...
    protected void initView() {
        ...
        loadData(getVideoUrl(index + "", Url.VideoJingPinId));
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("VideoJingPinFragment" + currentPagte);
    ...
    public void onRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                currentPagte = 1;
                isRefresh = true;
                loadData(getVideoUrl(0 + "", Url.VideoJingPinId));
            }
        }, 2000);
    }
    ...
    public void getResult(String result) {
        getMyActivity().setCacheStr("VideoJingPinFragment" + currentPagte, result);
        if (isRefresh) {
            isRefresh = false;
            videoAdapter.clear();
            listsModles.clear();
