...
public class KeJiFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    protected void initView() {
        ...
        loadData(getCommonUrl(index + "", Url.KeJiId));
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("KeJiFragment" + currentPagte);
    ...
    public void onRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                currentPagte = 1;
                isRefresh = true;
                loadData(getCommonUrl(0 + "", Url.KeJiId));
                url_maps.clear();
    ...
    public void getResult(String result) {
        getMyActivity().setCacheStr("KeJiFragment" + currentPagte, result);
        if (isRefresh) {
            isRefresh = false;
            newAdapter.clear();
            listsModles.clear();
