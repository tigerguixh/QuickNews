...
public class JunShiFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    protected void initView() {
        ...
        loadData(getCommonUrl(index + "", Url.JunShiId));
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("JunShiFragment" + currentPagte);
    ...
    public boolean isHasImgUrl(String imgUrl) {
        if (!"".equals(imgUrl)) {
            return true;
        }
        return false;
    ...
    public void onRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                currentPagte = 1;
                isRefresh = true;
                loadData(getCommonUrl(0 + "", Url.JunShiId));
                url_maps.clear();
    ...
    public void getResult(String result) {
        getMyActivity().setCacheStr("JunShiFragment" + currentPagte, result);
        if (isRefresh) {
            isRefresh = false;
            newAdapter.clear();
            listsModles.clear();
