...
public class QiCheFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    protected void initView() {
        ...
        loadData(getCommonUrl(index + "", Url.QiChiId));
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("QiCheFragment" + currentPagte);
    ...
    public void onRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                currentPagte = 1;
                isRefresh = true;
                loadData(getCommonUrl(0 + "", Url.QiChiId));
                url_maps.clear();
    ...
    public void getResult(String result) {
        getMyActivity().setCacheStr("QiCheFragment" + currentPagte, result);
        if (isRefresh) {
            isRefresh = false;
            newAdapter.clear();
            listsModles.clear();
